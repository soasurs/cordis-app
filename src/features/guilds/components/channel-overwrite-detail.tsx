...
import { ChannelOverwriteLabel } from '@/features/guilds/components/channel-overwrite-list'
import {
...
export function ChannelOverwriteDetail({
  onDeleted,
  overwrite,
  roles,
}: {
  ...
  return (
    <div className="overflow-hidden rounded-shell border border-line bg-surface-raised shadow-panel">
                ...
                {group.permissions.map((permission) => {
                  ...
                  return (
                    <li className="flex items-start gap-3 py-3" key={permission.value}>
                      ...
                      <ChannelOverwritePermissionToggle
                        disabled={busy}
      ...
      {!isDefaultEveryone ? (
        <ChannelOverwriteDeleteAction
          busy={busy}
          label={<ChannelOverwriteLabel overwrite={overwrite} roles={roles} />}
          onDelete={() => {
            saveMutation.reset()
            setSavedFlash(false)
...
function ChannelOverwriteDeleteAction({
  busy,
  label,
  onDelete,
  pending,
...
function ChannelOverwritePermissionToggle({
  disabled,
  label,
  onChange,
  state,
